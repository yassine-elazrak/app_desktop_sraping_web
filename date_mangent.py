...
class DIR:
    ...
    def ft_name_file(self):#########
        Label(self.fram, text="Name File Output", bg="#f2a343" , font =("Courier", 22, "italic")).grid(row=0, column=0, columnspan=2 ,sticky='w', padx=5, pady=2)
    ...
    def ft_path(self):
        Label(self.fram, text="Path of File Output", bg="#f2a343" , font =("Courier", 22, "italic")).grid(row=0, column=4, columnspan=4 ,sticky='W', padx=5, pady=2)
    ...
    def box_type_file(self):
        Checkbutton(self.fram, text=str("Format file output csv") ,variable=self.var_csv, command=self.change_csv, bg="#091833" , font =("Courier", 18, "italic")).grid(row=1, column=0, padx=0, pady=0)
        Checkbutton(self.fram, text=str("Format file output josn") ,variable=self.var_json, command=self.change_json, bg="#091833" , font =("Courier", 18, "italic")).grid(row=1, column=4,columnspan=2, padx=0, pady=0)
        self.var_csv.set(1)
        

    def get_all(self):

        if not path.isdir(self.path_file.get()):
    ...
    def main(self):
        self.fram = LabelFrame(self.master, text=" Enter Details For Output :" ,fg = "red", font =("Courier", 36, "italic"))
		#  font = "Times")
        self.fram.grid(row=2, columnspan=15, sticky='w', \
                 padx=5, pady=5, ipadx=5, ipady=5)
